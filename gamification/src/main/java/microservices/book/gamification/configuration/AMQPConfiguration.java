...
public class AMQPConfiguration {
    ...
    public TopicExchange challengesTopicExchange(@Value("${amqp.exchange.attempts}") final String exchangeName) {
        // While this is a consumer, it is good practice declaring the exchange
        // on both sides to have independent services.
        // Creating exchanges on RabbitMQ is idempotent, so it does not break the app.
    ...
    public Queue gamificationQueue(@Value("${amqp.queue.gamification}") final String queueName) {
        return QueueBuilder
                .durable(queueName)
                // .ttl((int) Duration.ofHours(6).toMillis()) // TTL is 6 hours
                // .maxLength(25000) // max 25000 messages in the queue
                .build();
    }
