...
public class GameServiceImpl implements GameService {
    ...
    public GameResult newAttemptForUser(ChallengeSolvedEvent challenge) {
        // We give points only if it's correct - while this is handled by the channel,
        // it is better to have it here as well for independence from it (and clarity).
        if (challenge.isCorrect()) {
            ScoreCard scoreCard = new ScoreCard(challenge.getUserId(), challenge.getAttemptId());
            scoreRepository.save(scoreCard);
            log.info("User {} score {} points for attempt id {}",
            ...
            return new GameResult(scoreCard.getScore(),
                    badgeCards.stream().map(BadgeCard::getBadgeType).collect(toList())
            );
        } else {
